...
var api = {
	 ...
	 */
	supported: function() {
		return supportedBrowsers.map(function(current) {
			return current.name;
		})
	},
	...
	getBrowser: function (name) {
		var self = this;
		var browser = _.find(supportedBrowsers, function (current) {
			return current.name === name;
		});

		if(self.cache[name]) {
			// If we have it cached return a promise that immediately fullfills
		...
		return new RSVP.Promise(function (resolve, reject) {
			...
			return self.getVersion(current);
		}).then(function(current) {
			self.cache[name] = current;
			return current;
		});
	}
};
